                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class="btn-action-main">
                                <a href="{{ $urlPrevious }}" class="btn">← メール設定</a>
                        ...
                        <div class="card-body mailset-wrap">
                            <form role="form" method="POST">
                                <div class="mailset-section-wrap">
                                    ...
                                    <div class="mailset-section mailset-enable"
                                        v-bind:class="data.setting_status == 2 ? 'blocked' : ''">
                                        <div class="switch">
                                            <span>無効</span>
    ...
    window.vue = new Vue({
        ...
        data: {
            ...
            data:{
                ...
                mail_setting_id: "{{$data['mail_setting_id']}}",
                setting_name: "{{$data['setting_name']}}",
                remarks: "{{$data['remarks']}}",
                setting_status: "{{$data['setting_status']}}",
                status: "{{$data['status']}}",
                receive_property: [
                    {{$data['receive_property']}}
                ],
                customer_target: [
                    {{$data['customer_target']}}
                ],
                ltv_from: "{{$data['ltv_from']}}",
                ltv_to: "{{$data['ltv_to']}}",
                purchased_date_from: "{{$data['purchased_date_from']}}",
                purchased_date_to: "{{$data['purchased_date_to']}}",
                first_time_purchased_date_from: "{{$data['first_time_purchased_date_from']}}",
                first_time_purchased_date_to: "{{$data['first_time_purchased_date_to']}}",
                last_time_purchased_date_from: "{{$data['last_time_purchased_date_from']}}",
                last_time_purchased_date_to: "{{$data['last_time_purchased_date_to']}}",
                purchased_times_from: "{{$data['purchased_times_from']}}",
                purchased_times_to: "{{$data['purchased_times_to']}}",
                cumulative_of_earnings_from: "{{$data['cumulative_of_earnings_from']}}",
                cumulative_of_earnings_to: "{{$data['cumulative_of_earnings_to']}}",
                old_from: "{{$data['old_from']}}",
                old_to: "{{$data['old_to']}}",
                standard_id: "{{$data['standard_id']}}",
                sex: [],
                prefectures: [],
                is_all_or_one_purchase: "{{$data['is_all_or_one_purchase']}}",
                is_buyed: "{{$data['is_buyed']}}",
                is_all_or_one_stop: "{{$data['is_all_or_one_stop']}}",
                schedules: [],
                schedules_remove: [],
                array_purchased_product_code: [],
                array_stop_product: [],
                flg_standard: "{{$data['flg_standard']}}",
                product_specify: {
                    model: "{{$data['product_specify']['model']}}",
                    name: "{{$data['product_specify']['name']}}",
                },
            },
            paramSearchProduct: {
                key_search: '',
            ...
            paramSearchProductPurchased: {key_search: '', product_name: 1, product_code: 1, product_jan: 1, price_from: '', price_to: ''},
            // data search product stop
            firstSearchStop: true,
            paramSearchProductStop: {key_search: '', product_name: 1, product_code: 1, product_jan: 1, price_from: '', price_to: ''},
            // data search product specific
            firstSearchSpecific: true,
            paramSearchProductSpecific: {key_search: '', product_name: 1, product_code: 1, product_jan: 1, price_from: '', price_to: ''},
            ...
            optionChangeLimit: [],
            idDateSchedule: 1,
            numSchedule: {{count($data['schedules']) ?? 1}}
        },
        ...
        methods: {
            ...
            */
            handleChangeCheckbox(){
                ...
                $("input[name='crm_condition_sel[]").change(function(e){
                    ...
                    if(checked && _this.data.crm_condition_sel.length  == $("input[name='crm_condition_sel[]").length ){
                        _this.checkAllCRMCondition = true;
                ...
                if(_this.data.customer_target.length == '11'){
                    _this.checkAllCRMCondition = true;
                    $("#form_crm_condition_sel_all").prop("checked", true);
                }
            },
            ...
            */
            handleCallOne(){
                ...
                $("#checkbox_all").change(function(){
                    var listProduct= $("#select-product tbody input[type='checkbox']:not('#checkbox_all')");
                ...
                //Change status senario
                $("input[name='setting_status']").on("change", function(e){
                    e.preventDefault();
                    if($(this).is(":checked")){
                        _this.data.setting_status = 1;
                    }else{
                        _this.data.setting_status = 0;
                    }
                })
            ...
            */
            handleSelectBaseDate(){
                ...
                for(var index = 0; index < isAfter.length; index++){
                    ...
                    // if(el.val() == 5) {
                    //     $(".timeline").addClass("standard-cart");
                    // }else{
                    //     $(".timeline").removeClass("standard-cart");
                    // }
                }
            },
            /**
                        ...
                        if(result.total > 0) {
                            ...
                            switch (_this.trackingPopupSelect) {
                                case 1:
                                    ...
                                    _this.paramSearchProductPurchased = {..._this.paramSearchProduct};
                                    break;
                                case 2:
                                    _this.listSearchProductStop = result;
                                    _this.paramSearchProductStop = {..._this.paramSearchProduct};
                                    break;
                                case 3:
                                    _this.listSearchProductSpecific = result;
                                    _this.paramSearchProductSpecific = {..._this.paramSearchProduct};
                                    break;
                                default:
                                    break;
                            }
            ...
            handleBtnChangeProduct(){
                ...
                switch (typeProduct) {
                        ...
                        case 2:
                            var maxSelect = 0;
                            var limit = 0;
                            var listProductChecked = $("#select-product tbody input[type='checkbox']:not('#checkbox_all'):checked");
                            if(typeProduct == 1){
                                maxSelect = this.limitProductPurchased - this.data.array_purchased_product_code.length;
            ...
            */
            handeSelectProduct(){
                ...
                $("#select-product tbody input[type='checkbox']:not('#checkbox_all')").on('click', function(){
                    var listSelect = $("#select-product input[type='checkbox']:not('#checkbox_all'):checked");
            ...
            */
            handleSave(){
                ...
                var offAllSchedule = true;
                if(listTimeline.length > 0) {
                    _this.data.schedules = [];
                    _this.data.schedules_remove= [];
                    for(var index = 0; index < listTimeline.length; index++){
                        ...
                        if($(listTimeline[index]).hasClass("remove")){
                            item = {};
                            item.schedule_id   = $(listTimeline[index]).find("[name='schedule_id']").val();
                            _this.data.schedules_remove.push(item);
                        }
                    }
                }
                var note = _this.data.setting_name;
                if(_this.data.setting_status == 1 && (_this.data.schedules.length == 0 || $(".timeline li.pending").length > 0 || $(".timeline li:not(.blocked) .btn-yellow").length > 0)){
                    note += '<br /> <span class="alert_action-warning">'+
                        ' <i class="fas fa-exclamation-triangle"></i> '+
                        'Can\'t on scenario, please select mail template for schedule'+
                        '<br /> If you must save then auto off scenario </span>';
                }
                if(_this.data.setting_status == 1 && offAllSchedule){
                    note += '<br /> <span class="alert_action-warning">'+
                        ' <i class="fas fa-exclamation-triangle"></i> '+
                        'Can\'t on scenario, please play one schedule'+
                        '<br /> If you must save then auto off scenario </span>';
                }
                alertAction('save scenario', note, function () {
                    $.ajax({
                        ...
                        success: function(result){
                            ...
                            if(result.status == 1)
                            {
                                if(result.arrError["setting_name"]) {
                                    $("input[name='setting_name']").parent().find(".msg-error").css("display", "block").addClass('error');
                                }
            ...
            */
            handleNamePopup(){
                ...
                if(this.trackingPopupSelect == 1) {
                    name = '商品の追加<p class="item-limit">【残り<span id="remaining_to_limit">'+(this.limitProductPurchased - this.data.array_purchased_product_code.length)+'</span>件追加可能】</p>';
                }
                if(this.trackingPopupSelect == 2) {
                    name = '配信停止ルール指定<p class="item-limit">【残り<span id="remaining_to_limit">'+(this.limitProductStop - this.data.array_stop_product.length)+'</span>件追加可能】</p>';
                }

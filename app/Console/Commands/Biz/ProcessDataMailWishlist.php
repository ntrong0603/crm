...
class ProcessDataMailWishlist extends ProcessBizCommand
{
    /**
     ...
     * @var string
     */
    protected $signature = 'biz_process:data-mail-wishlist';
    ...
    {
        $dataSchedule = (new DtMailSchedule)->getSchedule($mailType = 1, $standard_id = self::WISHLIST);
        $contentBody = '';
        if ($dataSchedule->count()) {
            foreach ($dataSchedule as $key => $schedule) {
                ...
                $products     = (new OcCart)->getDataWishlist($dateBase);
                ...
                if ($dataCustomer) {
                    foreach ($dataCustomer as $key => $customer) {
                        ...
                        $this->insertMailSent($dataSentMail, $customer, $action = "wishlist");
    ...
    public function renderHtml($dataProduct)
    {
        ...
        $data = [
            ...
            'param'            => '?utm_source=df_h&utm_medium=email&utm_campaign=favoritelist-mail&utm_content=html',
            'mall'             => 'biz',
        ];
        $html = view('admin.mail-template.sub-item.slot-data-body-wishlist', $data)->render();
        return $html;
    }
}

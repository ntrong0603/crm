...
class ProcessDataMailDateBuyLast extends ProcessBizCommand
{
    /**
     ...
     * @var string
     */
    protected $signature = 'biz_process:data-mail-date-buy-last';
     ...
     * @var string
     */
    protected $description = 'Command mapping data mail sent buy last';
    ...
    public function processMappingMail() {
        ...
        $dataSchedule = (new DtMailSchedule)->getSchedule($mailType = 1, $standard_id = self::BUYLAST);
        $contentBody = '';
        if($dataSchedule->count()) {
            foreach ($dataSchedule as $key => $schedule) {
                ...
                $dataCustomer = (new OcCustomer)->getCustomerLastBuy($date, $schedule);
